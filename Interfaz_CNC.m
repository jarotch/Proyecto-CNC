...
function abrirMuestreo(~, ~)
        ...
        figVentana = figure('Name', 'Muestreo', 'Position', [300, 300, 300, 200], 'Color', [0, 0.91, 0.47]);
        
        % Agregar boton de muestreo

        btnMuestreo_X = uicontrol(figVentana, 'Style', 'pushbutton', 'String', 'Muestrear en X', 'Position', [62, 145, 180, 30], 'Callback', @ejecutarGcodeMuestra_X, 'BackgroundColor', [0.01, 0.67, 0.45], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnMuestreo_Y = uicontrol(figVentana, 'Style', 'pushbutton', 'String', 'Muestrear en Y', 'Position', [62, 110, 180, 30], 'Callback', @ejecutarGcodeMuestra_Y, 'BackgroundColor', [0.01, 0.67, 0.45], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
    
        % Agregar campos de entrada para la longitud de muestra y el tiempo con colores personalizados
        uicontrol(figVentana, 'Style', 'text', 'String', 'Longitud', 'Position', [20, 50, 70, 20], 'BackgroundColor', [0.01 0.67 0.45], 'FontSize', 13, 'ForegroundColor', [1, 1, 1]);
        editLongitud = uicontrol(figVentana, 'Style', 'edit', 'Position', [90, 50, 50, 20], 'BackgroundColor', [1, 1, 1], 'FontSize', 12, 'ForegroundColor', [0.01 0.67 0.45]);
        uicontrol(figVentana, 'Style', 'text', 'String', 'Tiempo antes de ejecuci�n (s):', 'Position', [20, 20, 70, 20], 'BackgroundColor', [0.01 0.67 0.45], 'FontSize', 13, 'ForegroundColor', [1, 1, 1]);
        editTiempo = uicontrol(figVentana, 'Style', 'edit', 'Position', [90, 20, 50, 20], 'BackgroundColor', [1, 1, 1], 'FontSize', 12, 'ForegroundColor', [0.01 0.67 0.45]);
    ...
    function abrirVentanaEmergente(~, ~)
        ...
        btnArriba = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '+y', 'Position', [120, 150, 50, 30], 'Callback', @ejecutarGcodeArriba, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnAbajo = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '-y', 'Position', [120, 90, 50, 30], 'Callback', @ejecutarGcodeAbajo, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnIzquierda = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '+x', 'Position', [170, 120, 50, 30], 'Callback', @ejecutarGcodeIzquierda, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnDerecha = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '-x', 'Position', [70, 120, 50, 30], 'Callback', @ejecutarGcodeDerecha, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        ...
        function ejecutarGcodeArriba(~, ~)
            ...
            if isnan(distancia) || isnan(tiempo)
                msgbox('Ingrese valores v�lidos para distancia y tiempo.', 'Error', 'error');
                return;
            end
            if distancia > 260
                msgbox('Ingrese valores v�lidos para distancia y tiempo.', 'Error', 'error');
...
function GUIEnglish()
    ...
    buttonColor = [0.4, 0.6, 0.9]; % Light blue
    selectedButtonColor = [0.4, 0.6, 0.9]; % Medium blue
    ...
    btnAdd = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Add G-code', 'Position', [450, 60, 180, 30], 'Callback', @addGcode, 'BackgroundColor', buttonColor, 'FontSize', 13);

    % Create a button to delete G-code commands from the list
    btnDelete = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Delete G-code', 'Position', [450, 120, 180, 30], 'Callback', @deleteGcode, 'BackgroundColor', deleteButtonColor, 'FontSize', 13);

    % Create a button to execute G-code commands
    btnExecute = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Execute G-code', 'Position', [450, 150, 180, 30], 'Callback', @executeGcode, 'BackgroundColor', executeGcodeButtonColor, 'FontSize', 13);

    % Create a button "Read Before Use" that displays instructions
    btnReadBeforeUse = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Read Before Use', 'Position', [450, 240, 180, 30], 'Callback', @showInstructions, 'BackgroundColor', instructionsButtonColor, 'FontSize', 13);

    % Create a button "Command List" that displays a list of predefined commands
    btnCommandList = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Command List', 'Position', [450, 270, 180, 30], 'Callback', @showCommandList, 'BackgroundColor', commandListButtonColor, 'FontSize', 13);

    % Create a button "User Guide" to open a PDF file
    btnUserGuide = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'User Guide', 'Position', [450, 300, 180, 30], 'Callback', @openUserGuide, 'BackgroundColor', userGuideButtonColor, 'FontSize', 13);

    % Agregar el botón "Desplazamiento Rapido" a la GUI
    btnDesplazamientoRapido = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Fast Forward', 'Position', [450, 460, 180, 30], 'Callback', @abrirVentanaEmergente, 'BackgroundColor', buttonColor, 'FontSize', 13);

     % Agregar el botón "Desplazamiento Lento" a la GUI
    btnDesplazamientoLento = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Slow Move', 'Position', [450, 430, 180, 35], 'Callback', @abrirVentanEmergente, 'BackgroundColor', buttonColor, 'FontSize', 13);

     % Realizar un muestreo
    btnMuestra = uicontrol(mainPanel, 'Style', 'pushbutton', 'String', 'Sampling', 'Position', [450, 400, 180, 30],'Callback', @abrirMuestreo, 'BackgroundColor',buttonColor, 'FontSize', 12);
 ...
 % Callback para el botón "Desplazamiento Rapido"
    function abrirVentanaEmergente(~, ~)
        ...
        % Agregar botones para las flechas con colores personalizados
        btnArriba = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '+y', 'Position', [120, 155, 50, 30], 'Callback', @ejecutarGcodeArriba, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnAbajo = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '-y', 'Position', [120, 85, 50, 30], 'Callback', @ejecutarGcodeAbajo, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnIzquierda = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '+x', 'Position', [170, 120, 50, 30], 'Callback', @ejecutarGcodeIzquierda, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnDerecha = uicontrol(figEmergente, 'Style', 'pushbutton', 'String', '-x', 'Position', [70, 120, 50, 30], 'Callback', @ejecutarGcodeDerecha, 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        
        % Agregar campos de entrada para la distancia y el tiempo con colores personalizados
        uicontrol(figEmergente, 'Style', 'text', 'String', 'Distance', 'Position', [20, 50, 70, 20], 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 13, 'ForegroundColor', [1, 1, 1]);
        editDistancia = uicontrol(figEmergente, 'Style', 'edit', 'Position', [90, 50, 50, 20], 'BackgroundColor', [1, 1, 1], 'FontSize', 12, 'ForegroundColor', [0.4, 0.7, 0.9]);
        uicontrol(figEmergente, 'Style', 'text', 'String', 'Time before ejecution (s):', 'Position', [20, 20, 70, 20], 'BackgroundColor', [0.4, 0.7, 0.9], 'FontSize', 13, 'ForegroundColor', [1, 1, 1]);
        editTiempo = uicontrol(figEmergente, 'Style', 'edit', 'Position', [90, 20, 50, 20], 'BackgroundColor', [1, 1, 1], 'FontSize', 12, 'ForegroundColor', [0.4, 0.7, 0.9]);
        ...
        function ejecutarGcodeArriba(~, ~)
            ...
            tiempo = str2double(get(editTiempo, 'String'));
            if isnan(distancia) || isnan(tiempo)
                msgbox('Please enter valid values for disctance and time.', 'Error', 'error');
                return;
            end
            ejecutarGcodeConPausa(sprintf('G00 Y%.2f', distancia), tiempo);
        end
...
function abrirMuestreo(~, ~)
        ...
        % Crear una nueva figura para la ventana emergente
        figVentana = figure('Name', 'Sampling', 'Position', [300, 300, 300, 200], 'Color', [0 0.91 0.47]);
        
        % Agregar boton de muestreo

        btnMuestreo_X = uicontrol(figVentana, 'Style', 'pushbutton', 'String', 'Sample on X', 'Position', [62, 145, 180, 30], 'Callback', @ejecutarGcodeMuestra_X, 'BackgroundColor',  [0.01 0.67 0.45], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        btnMuestreo_Y = uicontrol(figVentana, 'Style', 'pushbutton', 'String', 'Sample on Y', 'Position', [62, 110, 180, 30], 'Callback', @ejecutarGcodeMuestra_X, 'BackgroundColor',  [0.01 0.67 0.45], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
        %btnMuestreo_Z = uicontrol(figVentana, 'Style', 'pushbutton', 'String', 'Muestrear en Z', 'Position', [62, 75, 180, 30], 'Callback', @ejecutarGcodeMuestra_X, 'BackgroundColor',  [0.01 0.67 0.45], 'FontSize', 14, 'ForegroundColor', [1, 1, 1]);
    
        % Agregar campos de entrada para la longitud de muestra y el tiempo
        uicontrol(figVentana, 'Style', 'text', 'String', 'Distance:', 'Position', [95, 50, 70, 20], 'BackgroundColor', [0.01 0.67 0.45], 'FontSize', 13, 'ForegroundColor', [1, 1, 1]);
        editLongitud = uicontrol(figVentana, 'Style', 'edit', 'Position', [165, 50, 50, 20], 'BackgroundColor', [1, 1, 1], 'FontSize', 12, 'ForegroundColor', [0.01 0.67 0.45]);
        uicontrol(figVentana, 'Style', 'text', 'String', 'Time:', 'Position', [95, 20, 70, 20], 'BackgroundColor',  [0.01 0.67 0.45], 'FontSize', 13, 'ForegroundColor', [1, 1, 1]);
        editTiempo = uicontrol(figVentana, 'Style', 'edit', 'Position', [165, 20, 50, 20], 'BackgroundColor', [1, 1, 1], 'FontSize', 12, 'ForegroundColor', [0.01 0.67 0.45]);
    ...
    function ejecutarGcodeMuestra_Z(~, ~)
            Longitud = str2double(get(editLongitud, 'String'));
            tiempo = str2double(get(editTiempo, 'String'));
            if isnan(Longitud) || isnan(tiempo)
                msgbox('Please enter valid values for disctance and time.', 'Error', 'error');
            ...
            ejecutarGcodeConPausa(sprintf('G00 Z%.2f', mitadL1), tiempo);
            ejecutarGcodeConPausa(sprintf('G00 Z%.2f', mitadL2), tiempo);
    end    
